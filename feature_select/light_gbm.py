...
for fold_id, (train_idx, val_idx) in enumerate(kfold.split(train[feature_names], train[label])):
    ...
    y_val_pred = lgb_model.predict(
        X_val[feature_names], num_iteration=lgb_model.best_iteration_)
...
feature_importances.to_csv('../feature_importance/gbm_feature_importance.csv')
predict = pd.DataFrame(
    {'id': test['id'],
     "score": prediction}
)
print(' gbm f1 avg_score =  ', avg_score)
predict.to_csv('../prediction/gbm_prediction.csv', index=False)
# %%
